	...
	$(document).on('click','.single_last_ten_sale',function(){
		...
		$.ajax({
			...
			success:function(response) {
				...
				$('#last_10_table_name').html(response.table_name);
				if(response.order_type==1){
					order_type = 'Dine In';
					order_type_id = 1;
					invoice_type = 'A ';
...
/*	$(document).on('mouseenter','.header_part_left_left .outlet_holder .outlet_holder_moving',function(){
				...
				success:function(response) {
					...
					for (var key in order_object.items) {
						...
						if(single_item.item_type=='Kitchen Item'){
							...
							item_list_to_show += '<div class="fix">'+item_name+'</div>';
							item_list_to_show += '<div class="fix">'+current_condition+'</div>';
					...
					$('#kitchen_status_table').html(table_name);
	...
	$(document).on('click','.single_item',function(){
		...
		// if($('#order_for_item_'+menu_id).length>0){
		// 	swal('Item already remains in cart!');
		// 	return false;
		// }
		var item_name = $(this).find('.item_name').html();
		var item_price = $('#price_'+menu_id).html();
		...
		$('#modal_item_price_variable').html(item_price);modal_item_price_variable_without_discount
		...
		for (var key in originalMenu) {
			modifiers += '<div class="modal_modifiers" data-selected="unselected" id="modifier_'+originalMenu[key].menu_modifier_id+'">';
	...
	$('#table_button').on('click',function(){
		...
		$('.single_table_div[data-table-checked=busy]').css('border','2px solid #cfcfcf');
		$('.single_table_div[data-table-checked=busy] .busy_content').hide();
		$('.single_table_div[data-table-checked=busy]').attr('data-table-checked','unchecked');
		$('.single_table_div[data-table-checked=unchecked]').css('background-color','2px solid #ffffff');
		for (var i = 1; i < 99999; i++)
        window.clearInterval(i);
    	all_time_interval_operation();
		$('.single_table_holder .booked_for .hour').html('00');
		$('.single_table_holder .booked_for .minute').html('00');
		$('.single_table_holder .booked_for .second').html('00');
		$.ajax({
			...
			success:function(response) {
				...
				for (var key in response) {
					if($('#single_table_'+response[key].table_id).length>0){
						$('#single_table_'+response[key].table_id).attr('data-table-checked','busy');
						$('#single_table_'+response[key].table_id).css('border','2px solid #dc3545');
						$('#single_table_'+response[key].table_id).css('background-color','#ffffff');
						$('#single_table_'+response[key].table_id+' .busy_content').show();
						var booked_time = new Date(Date.parse(response[key].date_time));
						var now = new Date();

						var diff =  now - booked_time;
						var hours   = Math.floor(diff / 3.6e6);
					    var minutes = Math.floor((diff % 3.6e6) / 6e4);
					    var seconds = Math.floor((diff % 6e4) / 1000);
					    $('#single_table_'+response[key].table_id).parent().find('#booked_for_hour_'+response[key].table_id).html(hours);
					    $('#single_table_'+response[key].table_id).parent().find('#booked_for_minute_'+response[key].table_id).html(minutes);
					    $('#single_table_'+response[key].table_id).parent().find('#booked_for_second_'+response[key].table_id).html(seconds);
						
						var now2 = new Date();

						// add a day
						now2.setDate(now2.getDate() + 1);
						upTime2($('#single_table_'+response[key].table_id),seconds,minutes,hours);
			...
			success:function(response) {
				...
				for (var key in orders) {
					...
					last_10_orders += '<div class="single_last_ten_sale fix" id="last_ten_'+orders[key].id+'" data-selected="unselected">';
                        ...
                        last_10_orders += '<div class="third_column column fix">'+table_name+'</div>';  
                    last_10_orders += '</div>';
				}
...
function adjust_left_side_order_list(){
	...
	var remaining_height_for_list = (parseFloat(main_left_portion_height)-parseFloat(height_left_button_holder_and_header)-parseFloat(10)).toFixed(2);
	...
	$.ajax({
		...
		success:function(response) {
			...
			for (var key in response) {
				...
				$order_list_left += '<span id="open_orders_order_status_'+response[key].sales_id+'" style="display:none;">'+response[key].order_status+'</span><p style="font-size: 16px;font-weight: bold;text-align:center;">Order: '+order_name+'</p>';
				$order_list_left += '<p>Table: '+table_name+'</p>';
				$order_list_left += '<p>Waiter: '+waiter_name+'</p>';
				$order_list_left += '<p>Customer: '+customer_name+'</p>';
				$order_list_left += '</div>';
...
function get_details_of_a_particular_order_for_kot_modal(sale_id) {
	$.ajax({
		...
		success:function(response) {
			...
			$('#kot_modal_table_name').html(response.table_name);
...
function get_details_of_a_particular_order_for_modal(sale_id){
	$.ajax({
		...
		success:function(response) {
			...
			$('#order_details_table_name').html(response.table_name);
			if(response.order_type==1){
				order_type = 'Dine In';
				order_type_id = 1;
				order_number = 'A '+response.sale_no;
...
function get_details_of_a_particular_order(sale_id){
	$.ajax({
		...
		success:function(response) {
			...
			$("#delivery_charge").val(response.delivery_charge);
			$("#total_payable").html(response.total_payable);
			$(".order_holder").prepend('<span id="modification_'+sale_id+'" class="modification" style="display:none;">'+sale_id+'</span>');
