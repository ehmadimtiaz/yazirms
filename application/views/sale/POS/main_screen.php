...
foreach($food_menus as $single_menus){
    ...
    if(!empty(@getimagesize(base_url().'assets/POS/images/'.$single_menus->photo)) && $single_menus->photo!=""){
...
foreach($new_orders as $single_new_order){
    ...
    $from_time = strtotime($single_new_order->date_time);
    $minutes = round(abs($to_time - $from_time) / 60,2);
    $seconds = abs($to_time - $from_time) % 60;

    $order_list_left .= '<span id="open_orders_order_status_'.$single_new_order->sales_id.'" style="display:none;">'.$single_new_order->order_status.'</span><p style="font-size: 16px;font-weight: bold;text-align:center;">Order: '.$order_name.'</p>';
    $order_list_left .= '<p>Table: '.$single_new_order->table_name.'</p>';
    $order_list_left .= '<p>Waiter: '.$single_new_order->waiter_name.'</p>';
    $order_list_left .= '<p>Customer: '.$single_new_order->customer_name.'</p>';
    $order_list_left .= '</div>';
    ...
    <body>
        <div class="wrapper fix">
            <div class="top_header_part fix">
                ...
                <div class="header_part_left fix">
                    <button id="open_hold_sales"><i class="fas fa-folder-open"></i> Open Hold Sales</button>
                    <button id="help_button"><i class="fas fa-question-circle"></i> Help</button>
                    <button id="calculator_button"><i class="fas fa-calculator"></i> Calculator</button>
                ...
                <div class="header_part_right fix">
                    ...
                    <div class="header_single_button_holder" style="width:17%;">
                        <a href="<?php echo base_url(); ?>Dashboard/dashboard"><button style="float:right;"><i class="fas fa-sign-out-alt"></i> Dashboard</button></a>
                    </div>
        ...
        <div id="help_modal" class="modal">
            ...
            <div class="modal-content" id="modal_help_details">
                ...
                <h1 id="help_modal_header">Help</h1>
                <div class="help_modal_info_holder fix">

                    <p class="para_type_1" style="text-align: center; margin-bottom: 10px; color: #DD4B39;">Please allow popup of your browser to print Invoice and KOT <br/>
                    When you see that there refresh button right beside of running orders is red. You need to click on that button to refresh running orders to get update from kitchen.</p> 
                    <p class="para_type_1">In Search</p>
                    ...
                    <p class="para_type_1">Running Orders</p>
                    ...
                    <p class="para_type_1">KOT</p>
                    <p class="para_type_2">Use Print KOT button if you intend to not to use Kitchen Panel</p>
                    <p class="para_type_2">When an order is modified, you can choose items and Qty to print in KOT</p>
                    <p class="para_type_2">But for Kitchen Panel, no need to worry, software will take care of modifications</p>
